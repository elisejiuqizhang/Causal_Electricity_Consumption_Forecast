"""
PatchTST Model OOD Inference Script
Load pretrained PatchTST models and test them on identified OOD weather windows
"""
...
def load_model_and_scaler(model_path, scaler_path, input_channels, horizon, context_length,
                          d_model, n_heads, n_layers, patch_len, patch_stride, dropout):
    """Load pretrained PatchTST model and scaler"""
    model = PatchTSTModel(
...
def main():
    parser = argparse.ArgumentParser(description='PatchTST OOD Inference')
    ...
    parser.add_argument('--model_dir', type=str, 
                       default=os.path.join(ROOT_DIR, 'outputs', 'forecast1111', 'per_region', 'patchtst_single_train'),
                       help='Model directory')
    parser.add_argument('--training_config', type=str,
                       default='bs64_ep500_lr0.0001_tr0.93_vr0.07_pat20_esep0.0001',
                       help='Training configuration folder name')
    parser.add_argument('--seed', type=int, default=597, help='Random seed used during training')
    ...
    parser.add_argument('--output_dir', type=str,
                       default=os.path.join(ROOT_DIR, 'outputs', 'ood_inference', 'patchtst'),
    ...
    model_path = os.path.join(
        ...
        f"dm{args.d_model}_nh{args.n_heads}_nl{args.n_layers}_pl{args.patch_len}_ps{args.patch_stride}_inlen{args.input_length}_h{args.horizon}_scalerstandard",
        "non_overlap" if args.stride == args.horizon else f"overlap/stride{args.stride}/mean",
