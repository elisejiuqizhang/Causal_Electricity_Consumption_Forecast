...
OUTPUT_DIR = os.path.join(ROOT_DIR, 'outputs', 'forecast', 'per_region_only_meteo', 'gru_single_train')
...
parser.add_argument('--with_tgt', default='False', help='whether to include target columns in input sequences')
# data directory
...
# forecast parameters
parser.add_argument("--n_folds", type=int, default=15, help="Number of folds/window splits for time series cross validation")
parser.add_argument("--window_size", type=int, default=24*3*366, help="Window size for each fold (in hours) - will be the total train+test of this fold")
...
OUTPUT_DIR = os.path.join(OUTPUT_DIR, exp_name, training_info, feature_set, str(seed))
os.makedirs(OUTPUT_DIR, exist_ok=True)
...
for fold, (start_idx, end_idx) in enumerate(fold_windows):
    ...
    X_arr, Y_arr = construct_sequences_uni_output(train_df_fold, history_len=input_length, horizon=horizon, step_size=stride, target_col='TOTAL_CONSUMPTION', with_tgt=with_tgt)
    ...
    X_snap_arr, Y_snap_arr = construct_sequences_uni_output(eval_snap_df, history_len=input_length, horizon=horizon, step_size=stride, target_col='TOTAL_CONSUMPTION', with_tgt=with_tgt)
    ...
    model = GRUForecaster(input_size=X_arr.shape[1], hidden_size=d_model, num_layers=n_layers, dropout=dropout, output_size=1, horizon=horizon).to(device)
    ...
    trigger_times = 0
    global_step = 0
    model_file = os.path.join(OUTPUT_DIR_FOLD, 'best_model.pth')
    ...
    X_test_arr, Y_test_arr = construct_sequences_uni_output(test_df_fold, history_len=input_length, horizon=horizon, step_size=stride, target_col='TOTAL_CONSUMPTION', with_tgt=with_tgt)
