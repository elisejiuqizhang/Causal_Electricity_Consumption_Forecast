...
OUTPUT_DIR = os.path.join(ROOT_DIR, 'outputs', 'forecast', 'per_region_only_meteo', 'tcn_single_train')
...
# TCN model
parser.add_argument("--hidden_channels", type=int, default=64, help="Number of hidden channels in TCN")
parser.add_argument("--levels", type=int, default=4, help="Number of levels/layers in TCN")
parser.add_argument("--kernel_size", type=int, default=3, help="Kernel size for TCN")
parser.add_argument("--dilation_base", type=int, default=2, help="Dilation base")
...
os.makedirs(OUTPUT_DIR, exist_ok=True)
exp_name = f"hid{hidden_channels}_lvl{levels}_ks{kernel_size}_db{dilation_base}_inlen{input_length}_h{horizon}_scaler{scaler}"
