...
from utils.data_utils.datetime_utils import add_time_features
...
def build_windows(df: pd.DataFrame, input_cols: List[str], target_col: str,
    ...
    """
    # Ensure target_col is included exactly once for Y extraction
    ...
    D = len(input_cols)  # inputs occupy the first D columns
    for end in range(L, N - H + 1):
        X_list.append(values[end-L:end, :D])           # (L, D)
        Y_list.append(values[end:end+H, D if target_col in input_cols else len(input_cols)])  # last col slice
...
def prepare_city_frame(cfg: SeriesConfig, features: List[str], target_col="load", include_past_load=True) -> pd.DataFrame:
    ...
    # add separate past-load input so we never scale/modify target_col itself
...
if __name__ == "__main__":
    ...
    p.add_argument("--city-electricity-csv", action="append", required=True,
                   help="Repeatable: City=path/to/merged_city.csv (must include time, load columns)")
    p.add_argument("--city-meteo-csv", action="append", required=True,
                   help="Repeatable: City=path/to/merged_city.csv (must include weather columns)")
    ...
    p.add_argument("--model", choices=["gru","patchtst","lstm","tcn","mamba"], default="gru",
                help="Pick model backbone.")
    ...
    p.add_argument("--model-kwarg", action="append", default=[],
                help="Repeatable: key=value args passed to the model (e.g., d_model=128 embed=timeF hidden_dim=256)")

    p.add_argument("--include-past-load", action="store_true", default=True,
                help="If set, adds a separate 'load_past' column for inputs (target remains unscaled).")
