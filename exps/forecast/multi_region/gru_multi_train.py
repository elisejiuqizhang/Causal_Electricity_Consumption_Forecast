# Multi-region GRU forecasting script
# Load data from multiple cities/regions
# Train a GRU model to forecast electricity consumption for all regions simultaneously
# Time series cross validation (sliding window) is used
# Results are saved per region with aggregated metrics

import os, sys
ROOT_DIR = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))))
sys.path.append(ROOT_DIR)

OUTPUT_DIR = os.path.join(ROOT_DIR, 'outputs', 'forecast', 'multi_region', 'gru_multi_train')
...
# arg parse
import argparse
parser = argparse.ArgumentParser(description='GRU multi-region training experiment')
...
parser.add_argument("--d_model", type=int, default=64, help="Hidden dimension (GRU hidden_size)")
...
exp_name = f"dm{d_model}_nl{n_layers}_inlen{input_length}_h{horizon}_scaler{scaler}"
...
for fold, (start_idx, end_idx) in enumerate(fold_windows):
    ...
    # initialize model - num_targets = number of regions
    model = GRUForecaster(input_size=X_arr.shape[1], hidden_size=d_model, num_layers=n_layers, 
                          dropout=dropout, output_size=Y_arr.shape[1], horizon=horizon).to(device)
    ...
    for epoch in range(epochs):
        ...
        # tensorboard logging
        writer.add_scalar(f'Fold_{fold}/Train_Loss', epoch_train_loss, epoch)
        ...
        if epoch < val_warmup:
            ...
            print(f'  Epoch {epoch+1}/{epochs} - Train Loss: {epoch_train_loss:.6f}, Val Loss: {val_loss:.6f}, Time: {epoch_duration:.2f}s')
        else:
            if val_loss + early_stopping_eps < best_val_loss:
                print(f'  Epoch {epoch+1}/{epochs} - Train Loss: {epoch_train_loss:.6f}, Val Loss: {val_loss:.6f}, Time: {epoch_duration:.2f}s [IMPROVED]')
                best_val_loss = val_loss
                torch.save(model.state_dict(), os.path.join(OUTPUT_DIR_FOLD, model_file))
                trigger_times = 0
            else:
                trigger_times += 1
                print(f'  Epoch {epoch+1}/{epochs} - Train Loss: {epoch_train_loss:.6f}, Val Loss: {val_loss:.6f}, Time: {epoch_duration:.2f}s [No improvement: {trigger_times}/{early_stopping_patience}]')
        ...
        # preview on fixed eval snapshot every 10 epochs - loop over regions
        if (epoch+1) % 10 == 0:
            for reg in region_list:
                target_channel = region_list.index(reg)
                try:
                    col_idx = train_df_fold.columns.get_loc(f'{reg}_TOTAL_CONSUMPTION')
                    load_mean = scaler_all.mean_[col_idx] if scaler.lower()!='none' and scaler is not None else 0.0
                    load_std = scaler_all.scale_[col_idx] if scaler.lower()!='none' and scaler is not None else 1.0
                except:
    ...
    if test_df_fold.shape[0] < min_needed:
        ...
        # append NaN lists for metrics
        fold_test_maes.append([np.nan] * len(region_list))
        fold_test_rmses.append([np.nan] * len(region_list))
        fold_test_mapes.append([np.nan] * len(region_list))
        fold_test_smapes.append([np.nan] * len(region_list))
        fold_test_mases.append([np.nan] * len(region_list))
        continue
    ...
    if len(t_starts) != all_preds.shape[0]:
        print(f"  Warning: Number of windows mismatch: {len(t_starts)} vs {all_preds.shape[0]}")
    ...
    for w, t0 in enumerate(t_starts):
        for h_step in range(horizon):
            ...
            bags[t_idx].append(all_preds[w, :, h_step])  # (num_targets,)
    pred_idx = sorted(bags.keys())
    if aggregation_mode=="mean":
        agg_pred_norm = np.array([np.mean(bags[i], axis=0) for i in pred_idx], dtype=np.float32)  # (T, num_targets)
    elif aggregation_mode=="first":
        agg_pred_norm = np.array([bags[i][0] for i in pred_idx], dtype=np.float32)
    else:
        raise ValueError(f"Unknown aggregation_mode: {aggregation_mode}")
    
    # denormalize predictions and truths - note that the output is multi-output for multiple regions
    ...
    for i, region in enumerate(region_list):
        if scaler.lower()!='none' and scaler is not None:
            try:
                ...
                load_mean = scaler_all.mean_[col_idx]
                load_std = scaler_all.scale_[col_idx]
            except:
                # fallback to overall mean/std if column not found
                load_mean = scaler_all.mean_.mean()
                load_std = scaler_all.scale_.mean()
            agg_pred[:, i] = agg_pred_norm[:, i] * load_std + load_mean
        else:
    ...
    list_mase=[]
    for i, region in enumerate(region_list):
        pred_region = agg_pred[:, i]
        true_region = true_aligned[:, i]
        
        if np.isfinite(true_region).all() and np.isfinite(pred_region).all():
            diff = pred_region - true_region
            mae = float(np.mean(np.abs(diff)))
            rmse = float(np.sqrt(np.mean(diff**2)))
            mape = float(np.mean(np.abs(diff) / (np.abs(true_region) + 1e-8))) * 100.0
            smape = float(np.mean(2.0 * np.abs(diff) / (np.abs(pred_region) + np.abs(true_region) + 1e-8))) * 100.0
            
            # MASE
            try:
                col_idx = test_df_fold.columns.get_loc(f'{region}_TOTAL_CONSUMPTION')
                naive_forecast = test_df_fold.iloc[input_length - horizon : len(pred_idx) + input_length - horizon, col_idx].values
                mase_denominator = np.mean(np.abs(true_region - naive_forecast))
                mase = float(mae / (mase_denominator + 1e-8))
            except:
                mase = np.nan
            
            print(f"  Test set results for {region} - MAE: {mae:.4f}, RMSE: {rmse:.4f}, MAPE: {mape:.4f}%, SMAPE: {smape:.4f}%, MASE: {mase:.4f}")
        else:
            mae, rmse, mape, smape, mase = np.nan, np.nan, np.nan, np.nan, np.nan
            print(f"  Test set results for {region} - NaN values detected, skipping metrics")
    ...
    for i, region in enumerate(region_list):
        ...
        plt.plot(aligned_dt, agg_pred[:, i], label="Predicted Load", marker='o', alpha=0.7)
        plt.title(f"{region} - Total Load Forecast (H={horizon}, stride={stride})\nMAE={list_mae[i]:.4f}, RMSE={list_rmse[i]:.4f}")
...
with open(os.path.join(OUTPUT_DIR, 'overall_results.txt'), 'w') as f:
    ...
    for fold_idx in range(len(fold_test_maes)):
        fold_mae_strs = [f'{fold_test_maes[fold_idx][reg_idx]:.4f}' if not np.isnan(fold_test_maes[fold_idx][reg_idx]) else 'NaN' 
                         for reg_idx in range(len(region_list))]
        f.write(f'{fold_idx}\t' + '\t'.join(fold_mae_strs) + '\n')
    # compute average MAE per region (ignoring NaNs)
    f.write('Average MAE per region\n')
    avg_row = []
    for reg_idx in range(len(region_list)):
        reg_maes = [fold_test_maes[fold_idx][reg_idx] for fold_idx in range(len(fold_test_maes)) if not np.isnan(fold_test_maes[fold_idx][reg_idx])]
        avg_mae = np.mean(reg_maes) if len(reg_maes) > 0 else np.nan
        avg_row.append(f'{avg_mae:.4f}' if not np.isnan(avg_mae) else 'NaN')
    ...
    for fold_idx in range(len(fold_test_rmses)):
        fold_rmse_strs = [f'{fold_test_rmses[fold_idx][reg_idx]:.4f}' if not np.isnan(fold_test_rmses[fold_idx][reg_idx]) else 'NaN' 
                          for reg_idx in range(len(region_list))]
        f.write(f'{fold_idx}\t' + '\t'.join(fold_rmse_strs) + '\n')
    f.write('Average RMSE per region\n')
    avg_row = []
    for reg_idx in range(len(region_list)):
        reg_rmses = [fold_test_rmses[fold_idx][reg_idx] for fold_idx in range(len(fold_test_rmses)) if not np.isnan(fold_test_rmses[fold_idx][reg_idx])]
        avg_rmse = np.mean(reg_rmses) if len(reg_rmses) > 0 else np.nan
        avg_row.append(f'{avg_rmse:.4f}' if not np.isnan(avg_rmse) else 'NaN')
    f.write('\t' + '\t'.join(avg_row) + '\n')
    ...
    for fold_idx in range(len(fold_test_mapes)):
        fold_mape_strs = [f'{fold_test_mapes[fold_idx][reg_idx]:.4f}' if not np.isnan(fold_test_mapes[fold_idx][reg_idx]) else 'NaN' 
                          for reg_idx in range(len(region_list))]
        f.write(f'{fold_idx}\t' + '\t'.join(fold_mape_strs) + '\n')
    f.write('Average MAPE per region\n')
    avg_row = []
    for reg_idx in range(len(region_list)):
        reg_mapes = [fold_test_mapes[fold_idx][reg_idx] for fold_idx in range(len(fold_test_mapes)) if not np.isnan(fold_test_mapes[fold_idx][reg_idx])]
        avg_mape = np.mean(reg_mapes) if len(reg_mapes) > 0 else np.nan
        avg_row.append(f'{avg_mape:.4f}' if not np.isnan(avg_mape) else 'NaN')
    f.write('\t' + '\t'.join(avg_row) + '\n')
    ...
    for fold_idx in range(len(fold_test_smapes)):
        fold_smape_strs = [f'{fold_test_smapes[fold_idx][reg_idx]:.4f}' if not np.isnan(fold_test_smapes[fold_idx][reg_idx]) else 'NaN' 
                           for reg_idx in range(len(region_list))]
        f.write(f'{fold_idx}\t' + '\t'.join(fold_smape_strs) + '\n')
    f.write('Average SMAPE per region\n')
    avg_row = []
    for reg_idx in range(len(region_list)):
        reg_smapes = [fold_test_smapes[fold_idx][reg_idx] for fold_idx in range(len(fold_test_smapes)) if not np.isnan(fold_test_smapes[fold_idx][reg_idx])]
        avg_smape = np.mean(reg_smapes) if len(reg_smapes) > 0 else np.nan
        avg_row.append(f'{avg_smape:.4f}' if not np.isnan(avg_smape) else 'NaN')
    f.write('\t' + '\t'.join(avg_row) + '\n')
    ...
    for fold_idx in range(len(fold_test_mases)):
        fold_mase_strs = [f'{fold_test_mases[fold_idx][reg_idx]:.4f}' if not np.isnan(fold_test_mases[fold_idx][reg_idx]) else 'NaN' 
                          for reg_idx in range(len(region_list))]
        f.write(f'{fold_idx}\t' + '\t'.join(fold_mase_strs) + '\n')
    f.write('Average MASE per region\n')
    avg_row = []
    for reg_idx in range(len(region_list)):
        reg_mases = [fold_test_mases[fold_idx][reg_idx] for fold_idx in range(len(fold_test_mases)) if not np.isnan(fold_test_mases[fold_idx][reg_idx])]
        avg_mase = np.mean(reg_mases) if len(reg_mases) > 0 else np.nan
        avg_row.append(f'{avg_mase:.4f}' if not np.isnan(avg_mase) else 'NaN')
    f.write('\t' + '\t'.join(avg_row) + '\n')
