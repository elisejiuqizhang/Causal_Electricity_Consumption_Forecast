...
OUTPUT_DIR = os.path.join(ROOT_DIR, 'outputs', 'forecast', 'multi_region', 'tcn_multi_train')
...
# arg parse
import argparse
parser = argparse.ArgumentParser(description='TCN multi-region training experiment')
...
for fold, (start_idx, end_idx) in enumerate(fold_windows):
    ...
    # initialize model
    model = TCNModel(input_channels=X_arr.shape[1], output_horizon=horizon, num_targets=Y_arr.shape[1], 
                        hidden_channels=hidden_channels, levels=levels, kernel_size=kernel_size, 
                        dilation_base=dilation_base, dropout=dropout).to(device)
