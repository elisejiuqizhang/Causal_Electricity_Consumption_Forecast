...
class ForecastHead(nn.Module):
    ...
    def forward(self, h_last):  # (B, d_model) -> (B,C_out,H)
        y = self.proj(h_last) # (B, H*C)
        return y.view(-1, self.num_cities, self.horizon)  # (B, C_out, H)
