...
class PerChannelLoss(nn.Module):
    ...
    def forward(self, pred, tgt):
        """ Shape: (bs, horizon, chnl)"""
        
        assert len(pred.shape) == 3, "pred should have dimensions : (batch size, horizon, channels)"
        assert len(tgt.shape) == 3, "target should have dimensions : (batch_size, horizon, channels)"
        assert pred.shape[0] == tgt.shape[0], "batch size (dim0) should match"
        assert pred.shape[2] == tgt.shape[2], "channel size (dim2) should match"
        assert pred.shape[1] == tgt.shape[1], "horizon size (dim1) should match"

        diff = pred - tgt  # Shape: (bs, horizon, channels)

        # mean over batch and horizon for each channel
        # output shape: (C,)
        if self.loss_type in ['mse', 'l2']:
            per_ch = diff.pow(2).mean(dim=(0, 1))
        elif self.loss_type in ['mae', 'l1']:
            per_ch = diff.abs().mean(dim=(0, 1))
        else:  # rmse
            per_ch = torch.sqrt(diff.pow(2).mean(dim=(0, 1)) + self.eps)
